                        ...
                        <form method="GET" class="w-full md:w-auto">
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    ...
                                    placeholder="Cari karyawan..." onchange="this.form.submit()">
                            </div>
                        </form>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-300 dark:divide-gray-600">
                                @forelse ($karyawans as $karyawan)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ ucfirst($karyawan->user->role->name) }}
                                        </td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ $karyawan->detail_jabatan }}
                                        </td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <form action="{{ route('manajer.karyawan.updateStatus', $karyawan->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <select name="status" onchange="this.form.submit()"
                                                    ...
                                                    <option value="Aktif"
                                                        {{ $karyawan->status === 'Aktif' ? 'selected' : '' }}>Aktif
                                                    </option>
                                                    <option value="Non-Aktif"
                                                        {{ $karyawan->status === 'Non-Aktif' ? 'selected' : '' }}>
                                                        Non-Aktif</option>
                                                </select>
                                            </form>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <a href="{{ route('penilaian.karyawan.create', ['karyawan_id' => $karyawan->id]) }}"
                                                    class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300"
                                                    title="Buat Penilaian">
                                                    <x-heroicon-o-clipboard-document-list class="h-5 w-5" />
                                                </a>

                                                <a href="{{ route('manajer.karyawan.show', $karyawan->id) }}"
                                                    class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300"
                                                    title="Detail">
                                                    <x-heroicon-o-eye class="h-5 w-5" />
                                                </a>
                                                <form action="{{ route('manajer.karyawan.destroy', $karyawan->id) }}"
