<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('penilaian.karyawan.store') }}">
                        @csrf
                        ...
                        <div id="print-area">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                ...
                                <div>
                                    ...
                                    <select name="karyawan_id" id="karyawan_id" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                        ...
                                        @foreach($karyawans as $k)
                                            <option value="{{ $k->id }}"
                                                ...
                                                data-jabatan="{{ $k->user->role->name }}"
                                                data-detail_jabatan="{{ $k->detail_jabatan }}"
                                                data-departemen="{{ $k->departemen }}">
                                                {{ $k->nip }} - {{ $k->nama }} ({{ $k->departemen }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="nip" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>
                                <div>
                                    <label for="nama" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama</label>
                                    <input type="text" id="nama" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>
                                <div>
                                    <label for="status" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status</label>
                                    <input type="text" id="status" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>
                                <div>
                                    <label for="jabatan" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jabatan</label>
                                    <input type="text" id="jabatan" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>
                                <div>
                                    <label for="detail_jabatan" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Detail Jabatan</label>
                                    <input type="text" id="detail_jabatan" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>
                                <div>
                                    <label for="departemen" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Departemen/Divisi</label>
                                    <input type="text" id="departemen" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                </div>

                                <!-- Penilai (auto-filled from logged in manager) -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Penilai</label>
                                    <input type="text" value="{{ $penilai->nama }}" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" readonly>
                                    <input type="hidden" name="penilai_id" value="{{ $penilai->id }}">
                                </div>

                                <div>
                                    <label for="periode" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Periode</label>
                                    <input type="text" name="periode" id="periode" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                </div>
                                <div>
                                    <label for="tanggal_penilaian" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Penilaian</label>
                                    <input type="date" name="tanggal_penilaian" id="tanggal_penilaian" value="{{ now()->format('Y-m-d') }}" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" required>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-4 justify-start text-sm mb-8">
                                <div class="w-80 flex border dark:border-gray-300 rounded overflow-hidden">
                                    ...
                                    <div class="w-1/2 text-center flex items-center justify-center font-semibold text-lg bg-white dark:bg-gray-700" id="result-persentase">0%</div>
                                </div>
                                <div class="w-80 flex border dark:border-gray-300 rounded overflow-hidden">
                                    <div class="bg-cyan-500 text-white font-bold w-1/2 text-center py-6 text-lg">Score</div>
                                    <div class="w-1/2 text-center flex items-center justify-center font-semibold text-lg bg-white dark:bg-gray-700" id="result-score">0</div>
                                </div>
                                <div class="w-80 flex border dark:border-gray-300 rounded overflow-hidden">
                                    <div class="bg-cyan-500 text-white font-bold w-1/2 text-center py-6 text-lg">Indeks</div>
                                    <div class="w-1/2 text-center flex items-center justify-center text-2xl font-bold text-blue-700 bg-white dark:bg-gray-700" id="result-indeks">-</div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" value="{{ $penilai->nama }}" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-center font-bold" readonly>
                                    </div>
                                    <div class="mb-2">
                                        <p class="text-sm text-gray-600 dark:text-gray-300">Tanggal</p>
                                        <input type="text" id="display_tanggal_penilaian" value="{{ now()->format('d-m-Y') }}" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-center font-bold" readonly>
                                    </div>
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="mb-2">
                                        <p class="text-sm text-gray-600 dark:text-gray-300">Nama</p>
                                        <input type="text" name="nama_mengetahui" id="nama_mengetahui" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-center font-bold">
                                    </div>
                                    <div class="mb-2">
                                        <p class="text-sm text-gray-600 dark:text-gray-300">Tanggal</p>
                                        <input type="text" id="display_tanggal_mengetahui" value="{{ now()->format('d-m-Y') }}" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-center font-bold" readonly>
                                    </div>
                                    <div class="mt-12 h-0.5 bg-gray-300 dark:bg-gray-600"></div>
                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Tanda tangan</p>
                                </div>
                                ...
                                <div>
                                    <div class="mb-6">
                                        <p class="font-bold mb-2 text-gray-800 dark:text-white">Corrective Action / Kebutuhan Training</p>
                                        <textarea name="corrective_action" rows="3" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
                                    </div>
                                    <div>
                                        <p class="font-bold mb-2 text-gray-800 dark:text-white">Feedback dari Karyawan terkait</p>
                                        <textarea name="feedback_karyawan" rows="3" class="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex justify-end gap-4 mt-8 no-print">
                            <button type="button" onclick="resetAktual()" class="bg-gray-600 text-white px-4 py-2 rounded text-xs font-semibold hover:bg-gray-700 uppercase tracking-wider transition-colors">Reset</button>
                            <button type="button" onclick="printTable()" class="bg-green-600 text-white px-4 py-2 rounded text-xs font-semibold hover:bg-green-700 uppercase tracking-wider transition-colors">Print</button>
                            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded text-xs font-semibold hover:bg-blue-700 uppercase tracking-wider transition-colors">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        // Update displayed dates when assessment date changes
